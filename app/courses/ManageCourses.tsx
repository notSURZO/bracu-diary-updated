...
export default function CoursesPage() {
  ...
  const fetchAndSyncCourses = async () => {
    ...
    setCourses(allCourses.filter((c: Course) => !enrolled.some((sc: Course) => sc._id === c._id)));
  };
              ...
              {filteredCourses.length === 0 ? (
                ...
                filteredCourses.map(course => (
                  <li key={course._id} className="relative flex items-center gap-4 bg-gradient-to-br from-white via-indigo-50 to-fuchsia-50 rounded-2xl px-6 py-4 font-medium shadow-lg hover:shadow-2xl transition group border border-indigo-100">
                    ...
                    <button
                      className="ml-2 px-6 py-2 bg-gradient-to-r from-green-400 to-green-600 rounded-xl text-white font-extrabold text-base shadow-lg hover:scale-110 hover:from-green-500 hover:to-green-700 transition flex items-center gap-2 border-2 border-green-200"
